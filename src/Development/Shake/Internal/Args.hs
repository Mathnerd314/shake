...
shakeOptsEx =
    ...
    ,yes $ Option "l" ["lint"] (noArg $ \s -> s{shakeLint=LintBasic}) "Perform limited validation after the run."
    ,yes $ Option ""  ["lint-fsatrace"] (noArg $ \s -> s{shakeLint=LintFSATrace}) "Use fsatrace to do validation."
    ,yes $ Option ""  ["no-lint"] (noArg $ \s -> s{shakeLint=LintNothing}) "Turn off --lint."
    ...
    ,yes $ Option ""  ["touch"] (noArg $ \s -> s{shakeRunCommands=RunMinimal}) "Mark targets as clean."
